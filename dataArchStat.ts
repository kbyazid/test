...
// Budgets data with transactions
...
// Vérification que le total des transactions ne dépasse pas le budget
budgets.forEach(budget => {
  const total = budget.transactions.reduce((sum, transaction) => sum + transaction.amount, 0);
  if (total > budget.amount) {
    console.warn(`Attention: Le budget ${budget.name} (${budget.amount}) est dépassé par les transactions (${total})`);
  }
});
